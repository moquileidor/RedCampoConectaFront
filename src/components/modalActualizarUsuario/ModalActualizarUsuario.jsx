  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (esCreacionNueva) {
        ...
        try {
          const { data: datosNuevos } = await api.post('/datosPersonales', formData);
          ...
          setMensaje({ texto: `Error al crear datos personales: ${error.response?.data?.message || error.message}`, tipo: 'danger' });
          return;
        }
      }
      ...
      try {
        ...
        setMensaje({ texto: `Error al actualizar usuario: ${error.response?.data?.message || error.message}`, tipo: 'warning' });
      }
      ...
      try {
        ...
        const { data: datosActualizados } = await api.put(`/datosPersonales/${userData.idDatos}`, formData);
        ...
        setMensaje({ texto: `Error al actualizar datos personales: ${error.response?.data?.message || error.message}`, tipo: 'danger' });
      }
    } catch (error) {
      setMensaje({ texto: `Error al procesar la operación: ${error.response?.data?.message || error.message}`, tipo: 'danger' });
    }
  };
